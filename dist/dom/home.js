...
socket.on("connect", () => showNotification("Connected To Server"));

socket.on("newMessage", (obj) => {
  const currentUserName = sessionStorage.getItem("userName");
  const arr = [];
  obj.isCurrentUser = obj.username === currentUserName;
  arr.push(obj);
  console.log(obj.username, currentUserName);
  createChats(arr);
});
...
async function createAdmin(id) {
  ...
  try {
    const response = await axios.put(
      `${backendAPI}/group/updateGroupAdmin`,
      myObj,
      { headers: { Authenticate: token } }
    );
    window.alert("Success");
...
function createChats(chats) {
  ...
  chats.forEach((data) => {
    if (!data.isCurrentUser) {
      ...
      const ownMessages = document.createElement("div");
